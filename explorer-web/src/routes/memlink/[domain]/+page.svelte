...
<div class="flex flex-col gap-6">
	...
	<div class="border-b border-slate-800 pb-4">
		...
		<h1 class="text-2xl font-bold text-slate-50 mt-1 select-all">{page.params.domain}</h1>
	</div>
		...
		<div class="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col gap-4">
			...
			<dl class="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs font-mono">
				...
				<div class="flex flex-col gap-1 md:col-span-4 bg-slate-950/40 border border-slate-750 p-3 rounded-lg">
					<span class="text-slate-500 uppercase text-[9px]">Raw TXT Value</span>
					{#if data.RawTXT}
						<span class="text-slate-300 font-bold select-all break-all">{data.RawTXT}</span>
					{:else}
				...
				<!-- MemNS Pointer -->
				<div class="flex flex-col gap-1 md:col-span-2 bg-slate-950/20 border border-slate-750/60 p-3 rounded-lg">
					<span class="text-slate-500 uppercase text-[9px]">Resolved MemNS Pointer</span>
					...
					{:else}
						<span class="text-slate-600 italic mt-1">no mutable name bound</span>
					{/if}
				</div>

				<!-- Resolved Target MID -->
				<div class="flex flex-col gap-1 md:col-span-2 bg-slate-950/20 border border-slate-750/60 p-3 rounded-lg">
					<span class="text-slate-500 uppercase text-[9px]">Resolved Destination MID</span>
					{#if data.ResolvedMID}
						<a 
					...
					{:else}
						<span class="text-slate-650 italic mt-1">no direct target content ID</span>
					{/if}
				</div>

				<div class="flex flex-col gap-1 mt-2">
					<span class="text-slate-500 uppercase text-[9px]">Cache TTL Remaining</span>
					<span class="text-slate-300 text-sm font-bold">
						{#if data.TTLRemaining >= 0}
							{data.TTLRemaining} seconds
						{:else}
							<span class="text-slate-600">expired / non-cached</span>
						{/if}
					</span>
				</div>
			</dl>
