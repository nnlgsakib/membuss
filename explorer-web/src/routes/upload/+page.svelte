...
<div class="max-w-2xl w-full mx-auto flex flex-col gap-6">
	...
	<div class="border-b border-zinc-800 pb-4">
		<h1 class="text-2xl font-black text-zinc-50">Upload Content to Membuss</h1>
		<p class="text-xs text-zinc-500 mt-1">Chunk, hash, erasure code, and index local files onto the network store</p>
	</div>
	...
	<div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-xl">
		{#if activeTab === 'file'}
			...
			<form onsubmit={handleFileSubmit} class="flex flex-col gap-6">
				<div class="flex flex-col gap-2">
					<label class="text-xs font-mono text-zinc-450 uppercase tracking-wider">Choose File</label>
					<div class="border-2 border-dashed border-zinc-800 hover:border-zinc-700/80 rounded-xl p-8 flex flex-col items-center justify-center gap-3 relative transition-all group bg-zinc-950/20">
						<span class="text-4xl group-hover:scale-110 transition-transform">📄</span>
						<div class="text-sm font-semibold text-zinc-300">
							{#if selectedFile}
								<span class="text-cyan-400">{selectedFile.name}</span>
								<span class="text-zinc-500 text-xs block font-mono font-normal mt-1">({formatBytes(selectedFile.size)})</span>
							{:else}
								<span>Select a file to upload</span>
							{/if}
						</div>
						<p class="text-xs text-zinc-650 max-w-xs text-center">
				...
				<button
					...
					class="w-full py-3 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 disabled:from-zinc-800 disabled:to-zinc-800 text-zinc-950 disabled:text-zinc-600 font-bold text-sm rounded-xl transition-all duration-300 shadow-[0_0_15px_rgba(6,182,212,0.15)] disabled:shadow-none"
				>
					Ingest File
				</button>
			</form>
		{:else}
			...
			<form onsubmit={handleFolderSubmit} class="flex flex-col gap-6">
				<div class="flex flex-col gap-2">
					<label class="text-xs font-mono text-zinc-450 uppercase tracking-wider">Choose Folder</label>
					<div class="border-2 border-dashed border-zinc-800 hover:border-zinc-700/80 rounded-xl p-8 flex flex-col items-center justify-center gap-3 relative transition-all group bg-zinc-950/20">
						<span class="text-4xl group-hover:scale-110 transition-transform">📁</span>
						<div class="text-sm font-semibold text-zinc-300">
							{#if selectedFiles && selectedFiles.length > 0}
								<span class="text-cyan-400">{selectedFiles.length} files selected</span>
								<span class="text-zinc-500 text-xs block font-mono font-normal mt-1">
				...
				<div class="flex flex-col gap-1.5">
					<label for="folder-name-input" class="text-xs font-mono text-zinc-450 uppercase tracking-wider">
					...
					<input
						...
						class="w-full bg-zinc-950/60 border border-zinc-800 text-zinc-200 text-sm px-4 py-2.5 rounded-lg focus:outline-none focus:border-cyan-500/80 focus:ring-1 focus:ring-cyan-500/20"
					/>
				</div>
	...
	<div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
		<div class="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col">
			...
			<div class="px-5 py-4 bg-zinc-950/40 border-b border-zinc-800/80 flex items-center justify-between">
				...
				{#if uploadPhase === 'uploading'}
					<button onclick={cancelUpload} class="text-xs text-zinc-550 hover:text-red-400 border border-zinc-800 hover:border-red-900/60 px-2 py-1 rounded transition-colors bg-zinc-950/40">
						Cancel
					</button>
				{/if}
			</div>
			...
			<div class="p-5 flex flex-col gap-4">
				...
				<div class="flex items-end justify-between font-mono">
					<span class="text-4xl font-black text-cyan-400 leading-none shadow-[0_0_20px_rgba(6,182,212,0.15)]">
						{uploadPercent}%
					</span>
				...
				<!-- Bar -->
				<div class="w-full h-2 rounded-full bg-zinc-950 border border-zinc-850 overflow-hidden">
					<div 
				...
				<div class="flex flex-col gap-1.5 mt-2">
					...
					<div class="bg-zinc-950/80 border border-zinc-850 rounded-lg max-h-36 overflow-y-auto divide-y divide-zinc-900/50 p-2 font-mono text-[10px] text-zinc-500">
						{#each uploadFileList as file}
							<div class="py-1 px-1 flex justify-between gap-4">
								<span class="truncate text-zinc-400 select-all" title={file.path}>{file.name}</span>
								<span class="shrink-0 text-zinc-600">{formatBytes(file.size)}</span>
							</div>
						{/each}
					</div>
				</div>
