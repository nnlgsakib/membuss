...
<div class="flex flex-col gap-6">
	...
	<div class="border-b border-zinc-800 pb-4 flex items-center justify-between">
		<div>
			...
			<h1 class="text-2xl font-black text-zinc-50 mt-1 break-all select-all">/{page.params.name}</h1>
		</div>
	</div>

	{#if loading && !data}
		<div class="space-y-6 animate-pulse">
			<div class="h-40 bg-zinc-900 rounded-lg"></div>
			<div class="h-32 bg-zinc-900 rounded-lg"></div>
		</div>
	{:else if error}
		...
		<div class="grid grid-cols-1 gap-6">
			...
			<div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex flex-col gap-4">
				...
				<dl class="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs font-mono">
					<div class="flex flex-col gap-1 md:col-span-4 bg-zinc-950/40 border border-zinc-850 p-3 rounded-lg">
						<span class="text-zinc-500 uppercase text-[9px]">Resolved Value</span>
						{#if data.IsMID}
							<a 
						...
						{:else}
							<span class="text-zinc-300 text-sm font-bold break-all">{data.Value}</span>
						{/if}
					</div>

					<div class="flex flex-col gap-1">
						<span class="text-zinc-500 uppercase text-[9px]">Sequence Number</span>
						<span class="text-zinc-200 text-sm font-bold">{data.Sequence}</span>
					</div>

					<div class="flex flex-col gap-1">
						<span class="text-zinc-500 uppercase text-[9px]">Remaining TTL</span>
						<span class="text-zinc-200 text-sm font-bold">{data.TTL}</span>
					</div>

					<div class="flex flex-col gap-1 md:col-span-2">
						<span class="text-zinc-500 uppercase text-[9px]">Record Expiration</span>
						<span class="text-zinc-200 text-sm font-bold">{formatDate(data.ExpiresAt)}</span>
					</div>
				</dl>
			</div>
			...
			{#if data.Routes && data.Routes.length > 0}
				<div class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
					<div class="px-6 py-4 bg-zinc-950/40 border-b border-zinc-800">
						<h3 class="font-bold text-sm text-zinc-300">Weighted Routing Rule Engine</h3>
					</div>
					<div class="overflow-x-auto">
						<table class="w-full text-left border-collapse text-sm">
							...
							<tbody class="divide-y divide-zinc-850/40 font-mono text-xs">
								{#each data.Routes as r}
									<tr class="hover:bg-zinc-850/25 transition-colors">
										<td class="py-3.5 px-6 font-semibold text-zinc-200">{r.label || 'n/a'}</td>
										<td class="py-3.5 px-6">
											<a href={`${base}/mid/${r.target}`} class="text-cyan-400 hover:underline">
												{r.target}
											</a>
										</td>
										<td class="py-3.5 px-6 text-zinc-300 text-right font-bold">{r.weight}%</td>
									</tr>
								{/each}
							</tbody>
						</table>
			...
			{#if data.Delegates && data.Delegates.length > 0}
				<div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex flex-col gap-4">
					...
					<ul class="flex flex-col gap-2 font-mono text-xs text-zinc-300">
						{#each data.Delegates as key}
							<li class="bg-zinc-950/60 border border-zinc-850 px-4 py-2.5 rounded-lg select-all break-all">
								{key}
							</li>
						{/each}
					</ul>
			...
			<div class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
				<div class="px-6 py-4 bg-zinc-950/40 border-b border-zinc-800">
					<h3 class="font-bold text-sm text-zinc-300">Publish History Timeline (MemLog)</h3>
				</div>
						...
						<table class="w-full text-left border-collapse text-sm">
							...
							<tbody class="divide-y divide-zinc-850/40 font-mono text-xs">
								{#each data.Changelog as entry}
									<tr class="hover:bg-zinc-850/25 transition-colors">
										<td class="py-3.5 px-6 text-zinc-400 font-bold">{entry.sequence}</td>
										<td class="py-3.5 px-6 text-zinc-500">{formatDate(entry.timestamp)}</td>
										<td class="py-3.5 px-6 break-all max-w-sm">
											...
											{:else}
												<span class="text-zinc-350">{entry.value}</span>
											{/if}
										</td>
										<td class="py-3.5 px-6 text-zinc-400 text-right font-sans italic">
											{entry.message || '—'}
										</td>
									</tr>
								{/each}
				...
				{:else}
					<div class="py-8 text-center text-zinc-550 flex flex-col items-center justify-center gap-1.5">
						<span>📜</span>
						<p class="text-xs text-zinc-500">No timeline events recorded for this record</p>
					</div>
				{/if}
			</div>
		</div>
